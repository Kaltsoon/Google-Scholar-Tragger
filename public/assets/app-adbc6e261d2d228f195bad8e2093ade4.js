(function(){
	...
	searchApp.controller("SearchController", function($scope){
		...
		$scope.register_click = function(result){
			if(current_query_id != null){
				$.post("/query_clicks", { scholar_query_id: current_query_id, heading: result.title, link_location: result.link, synopsis: result.synopsis, location: result.location, authors: result.authors });
			}
		}
